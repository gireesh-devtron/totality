...
func (impl *UserServerTester) RunTests() {
	...
	fmt.Println("test2 : get user by id 2")
	impl.testGetByUserId(2)
	fmt.Println("test3 : get user by id 3")
	impl.testGetByUserId(3)
	fmt.Println("test4 : get users by ids 1,2,3")
	impl.testGetByUserIds([]int32{1, 2, 3})
	fmt.Println("test5 : get user by id 4,5")
	impl.testGetByUserIds([]int32{4, 5})
	fmt.Println("test6 : get user by id 1000")
